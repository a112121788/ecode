...
| 组件 | 说明 |
|---|---|
| `src/ECode` | WPF 主应用，承载 Workspace、Surface、Pane、Browser Surface 与设置窗口。 |
| `src/ECode.Core` | Core 模型、终端缓冲区、ConPTY、IPC、配置、会话恢复与 Browser scripting contract。 |
| `src/ECode.Cli` | `ecode` CLI，负责本地命令、v1 pipe 兼容与 `ecode.v2` 请求。 |
...
3. CLI 通过 `\\.\pipe\ecode` 发送 v1 命令或 `ecode.v2` JSON 请求。
