...
ECode 是 Windows 原生 SuperTerminal：把终端 Workspace、Browser Surface、脚本化控制、会话恢复和 Windows 安装更新整合到一个桌面工作流中。
...
| `0.4.x` | Browser Pane 基础 | M3 | preview |
| `0.5.x` | Browser scripting API | M4 | beta |
| `0.6.x` | `ecode.v2`、多窗口、短引用 | M5 | beta |
| `0.7.x` | Shell / CLI 集成、自动更新、安装器 | M6 | 候选发布 |
...
- Browser Surface 持久化与分屏渲染。
- WebView2 Runtime 缺失提示。
- Browser toolbar、URL/title/history 状态与 CLI open 命令。
- `workspace.surfaces` 支持 Browser tabs。
...
- self-contained、CLI、Velopack、Inno Setup、MSIX 打包路径。

### M7 - 文档、社区与 1.0

- 简体中文用户文档：安装、快速上手、CLI、Browser API、会话恢复、故障排查。
...
- terminal / layout / notification / session restore / `ecode.json` / browser / v2 CLI 核心流程达到 beta 或 stable 质量。
