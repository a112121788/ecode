...
public sealed class PowerShellHookSetupService
{
    ...
    private static string CreateHookBlock(string installDirectory)
    {
        ...
        return string.Join(Environment.NewLine, new[]
        {
            ...
            "$ecodexContextArgs = @()",
            "if ($env:ECODEX_WORKSPACE_ID) { $ecodexContextArgs += @('--workspace-id', $env:ECODEX_WORKSPACE_ID) }",
            "if ($env:ECODEX_SURFACE_ID) { $ecodexContextArgs += @('--surface-id', $env:ECODEX_SURFACE_ID) }",
            "if ($env:ECODEX_PANE_ID) { $ecodexContextArgs += @('--pane-id', $env:ECODEX_PANE_ID) }",
            "if (Get-Command Set-PSReadLineOption -ErrorAction SilentlyContinue) {",
            "    Set-PSReadLineOption -AddToHistoryHandler {",
            "        param([string]$command)",
            "        $global:__ecodexLastCommand = $command",
            "        try { & $ecodexCli hook event --phase start --command $command --exit-code 0 --cwd (Get-Location).Path @ecodexContextArgs *> $null } catch {}",
            "        return $true",
            "    }",
            "}",
            ...
            "            try { & $ecodexCli hook event --phase end --command $global:__ecodexLastCommand --exit-code $exitCode --cwd (Get-Location).Path @ecodexContextArgs *> $null } catch {}",
            "            $global:__ecodexLastCommand = $null",
            "        }",
