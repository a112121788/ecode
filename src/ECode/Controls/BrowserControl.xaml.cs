...
public partial class BrowserControl : UserControl
{
    ...
    public async Task<BrowserScriptingSnapshot?> GetScriptingSnapshotAsync()
    {
        ...
        const string script = """
            (() => {
                let nextId = window.__ecodeNodeSeq || 0;
                window.__ecodeNodeSeq = nextId;

                function ensureNodeId(element) {
                    let id = element.getAttribute('data-ecode-node-id');
                    if (!id) {
                        nextId += 1;
                        id = `ecode-node-${nextId}`;
                        element.setAttribute('data-ecode-node-id', id);
                        window.__ecodeNodeSeq = nextId;
                    }
                    return id;
                }
    ...
    public async Task<BrowserScriptingActionOutcome> ExecuteScriptingActionAsync(BrowserScriptingActionRequest request)
    {
        if (WebView.CoreWebView2 == null)
        {
            return BrowserScriptingActionOutcome.FromError(
                ECode.Core.IPC.V2.V2ErrorCodes.NotFound,
        ...
        try
        {
            return request.Action switch
            {
                ...
                _ => BrowserScriptingActionOutcome.FromError(
                    ECode.Core.IPC.V2.V2ErrorCodes.NotSupported,
        ...
        {
            return BrowserScriptingActionOutcome.FromError(
                ECode.Core.IPC.V2.V2ErrorCodes.InternalError,
                ex.Message);
        }
    }
        ...
        string body)
    {
        ...
        {
            return BrowserScriptingActionOutcome.FromError(
                ECode.Core.IPC.V2.V2ErrorCodes.InvalidRef,
                "Browser action requires a target node.");
        }

        var script = $$"""
            (() => {
                const el = document.querySelector('[data-ecode-node-id={{CssString(request.Node.NodeId)}}]');
        ...
        return IsScriptOk(result)
            ...
            : BrowserScriptingActionOutcome.FromError(
                ECode.Core.IPC.V2.V2ErrorCodes.StaleRef,
