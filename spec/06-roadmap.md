...
ECode 的北极星是：**让 Windows 上的多项目终端、浏览器预览、脚本化控制和 AI 辅助开发形成稳定、可恢复、可审计的 SuperTerminal 工作台。**
...
截至 2026-06-15，ECode 已形成 `1.0.0` 稳定基线：
...
### Next：用 ECode 自身能力 dogfood AI 开发

目标：让 ECode 的 CLI / Browser / Restore / Notification 反过来支撑本仓库开发。

| Outcome | 说明 | 指标 |
|---|---|---|
| 本地 smoke 脚本化 | 用 `ecode.v2` 创建 workspace/pane/browser，跑常用开发命令 | 至少 1 条可复用 dogfood 脚本 |
| 自动证据收集 | 测试、截图、日志、health 输出能汇总到 PR note | PR 模板引用证据路径 |
| 配置模板化 | `.ecode/ecode.json` 提供 build/test/docs/release 常用命令 | 新贡献者 5 分钟可打开命令面板执行 |
