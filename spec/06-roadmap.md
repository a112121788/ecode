...
>
> 信息来源：
> 1. 当前 Windows 仓库源码：`src/Cmux`、`src/Cmux.Core`、`src/Cmux.Cli`、`src/Cmux.Daemon`、`tests/`、`scripts/`
> 2. 上游 macOS 原版：`manaflow-ai/cmux` README / TODO / CLI 目录 / 中文文档站
> 3. 上游公开文档：`https://cmux.com/zh-CN`、`/docs/getting-started`、`/docs/custom-commands`
>
...
把当前 cmux-windows 从“可用的 Windows 终端复用器原型”推进到“与 macOS cmux 核心体验对齐、面向 AI coding agents 的 Windows 原生开发工作台”。
...
3. 使用 `cmux.json` 把一个项目的常用布局、Agent 启动命令、浏览器预览和快捷按钮固化下来。
...
cmux-windows 不是一个新的 IDE，也不是强流程的 Agent 编排器，而是 Windows 上的一组可组合开发原语：
...
| 非目标 | 原因 |
|---|---|
| 替代 VS Code / JetBrains | cmux-windows 只提供 terminal/browser/control primitives，不管理编辑器工作流 |
...
| 在近期实现完整 remote daemon | `cmux ssh` 的 proxy broker / remote daemon 成本高，先列为远期评估项 |
...
| 不自动执行不可信命令 | resume / cmux.json command / hooks setup 均需要信任模型或显式确认 |
| 优先可测试 | 新服务应进 `Cmux.Core`，UI 只做绑定；协议先写 schema，再写实现 |
...
| 领域 | 已有能力 | 主要文件 |
|---|---|---|
| 终端引擎 | ConPTY 封装、Shell 启动、VT/CSI/OSC 解析、缓冲区、回滚、备用屏幕、宽字符、选择、URL 识别 | `Cmux.Core/Terminal/*` |
| 布局 | Workspace、Surface、Pane、二叉树分屏、关闭面板、焦点切换、缩放 | `Models/SplitNode.cs`、`ViewModels/*` |
| UI | 主窗口、侧边栏、标签栏、命令面板、通知面板、日志窗口、历史窗口、设置窗口、Session Vault | `src/Cmux/Views/*`、`Controls/*` |
...
| 守护进程 | `cmux-daemon.exe` 托管 `TerminalSession`、attach、snapshot、事件广播 | `src/Cmux.Daemon/*`、`DaemonClient.cs` |
| CLI | `notify / workspace / surface / split / status` | `src/Cmux.Cli/Program.cs` |
| 设置 | 外观、终端、行为、Agent、ShellProfiles、KeyBindings | `CmuxSettings.cs`、`SettingsWindow.xaml.cs` |
...
| 测试 | xUnit core tests、ConPTY smoke | `tests/Cmux.Tests`、`tests/Cmux.Smoke` |
...
| Custom commands | `cmux.json` commands/actions/layout/buttons | 缺失 | P0 | M1 基础，M3 浏览器 surface，M5 v2 |
| Scriptable socket API | 创建 workspace/pane、send keys、browser automation | v1 CLI 很窄 | P0 | M5 v2 协议 |
| SSH workspace | `cmux ssh` + remote browser route | 缺失 | P3 | 远期评估，不进主线 |
...
| `0.2.x` | UI/通知体验对齐、`cmux.json` 基础 | M1 | preview |
...
| M1 cmux.json 基础 | `CommandPalette`、`SplitNode`、`TerminalSession` | M3 browser layout、M6 hooks |
...
| M3 browser pane | WebView2、`SplitPaneContainer`、M1 cmux.json layout | M4 browser automation |
...
| M0-T03 | 统一版本源 | 从 `Cmux.csproj` Version 读取到 CLI 与 STATUS | `Cmux.Cli/Program.cs`、`MainViewModel.cs` | `cmux version` 与 `STATUS.version` 一致 |
| M0-T04 | 守护进程可观测性 | 标准化 `daemon-debug.log` 字段：component/event/paneId | `DaemonClient.cs`、`DaemonPipeServer.cs` | 日志可被 grep 追踪一次 attach |
| M0-T05 | 增补 VT parser tests | UTF-8 跨包、OSC ST、CSI private modes、invalid sequence | `tests/Cmux.Tests/CoreTests.cs` | 新增 ≥20 个 parser 测试 |
...
cmux-win-x64/cmuxw.exe   yes     5.4     ...
cmux-win-x64-sc/cmuxw.exe yes    82.1    ...
cmux-cli/cmux.exe        yes     72.8    ...
```

### M0.4 验收门槛

- `dotnet build Cmux.sln -c Debug` 通过。
- `dotnet test tests/Cmux.Tests/Cmux.Tests.csproj` 通过。
- `dotnet run --project tests/Cmux.Smoke/Cmux.Smoke.csproj` 在 Windows 机器上通过。
...
## M1 - UI/UX 与 cmux.json 基础

### M1.1 目标

对齐 macOS cmux 的可见核心体验：
...
1. 项目本地：`./.cmux/cmux.json`
2. 本地回退：`./cmux.json`
3. 全局：`~/.config/cmux/cmux.json`

Windows 路径映射：

1. `<workspace cwd>\.cmux\cmux.json`
2. `<workspace cwd>\cmux.json`
3. `%USERPROFILE%\.config\cmux\cmux.json`
...
| ID | 任务 | 说明 | 文件 |
|---|---|---|---|
| M1-C01 | 新增 DTO | `CmuxJsonConfig / CmuxCommand / CmuxAction` | `Cmux.Core/Models/CmuxJsonConfig.cs` |
| M1-C02 | 解析服务 | 路径搜索、JSON 解析、全局/本地 merge | `Cmux.Core/Services/CmuxJsonService.cs` |
...
| M1-C06 | reload config | CLI `cmux reload-config` + `Ctrl+Shift+,` | `Program.cs`、`MainWindow.xaml.cs` |
...
```csharp
public sealed class CmuxJsonConfig
{
    public Dictionary<string, CmuxAction> Actions { get; set; } = [];
    public List<CmuxCommand> Commands { get; set; } = [];
    public CmuxUiConfig? Ui { get; set; }
}

public sealed class CmuxAction
{
...
- `.cmux/cmux.json` 简单命令出现在命令面板；schema 错误在命令面板可见。
...
| `CmuxJsonService_MergesLocalOverGlobal` | xUnit |
| `CmuxJsonService_InvalidSchema_ReturnsDiagnostic` | xUnit |
...
上游文档强调：cmux 只恢复 app-owned state，不 checkpoint 任意 live process；对支持 agent 通过 hooks/native session id 恢复。Windows 版应采取同样原则：
...
建议新增 `%LOCALAPPDATA%/cmux/resume.json`：
...
| ID | 任务 | 说明 | 文件 |
|---|---|---|---|
| M2-T01 | `ResumeBinding` DTO | 数据模型 + JSON serializer | `Cmux.Core/Models/ResumeBinding.cs` |
| M2-T02 | `ResumeBindingService` | Load/Save/Add/Remove/FindForSurface/TrustPrefix | `Cmux.Core/Services/ResumeBindingService.cs` |
| M2-T03 | 敏感环境剔除 | TOKEN/PASSWORD/SECRET/API_KEY 等丢弃 | 同上 |
| M2-T04 | `CMUX_WORKSPACE_ID` 注入 | 启动 shell 时附带 env | `TerminalProcess.cs` |
| M2-T05 | CLI surface resume | `cmux surface resume set/show/clear` | `Cmux.Cli/Program.cs`、`MainViewModel.cs` |
| M2-T06 | 自动恢复开关 | `AutoResumeAgentSessions` | `CmuxSettings.cs`、`SettingsWindow.xaml` |
...
1. 来源是 cmux 内置可信检测（如当前 live process 检测到 tmux session，并生成 `tmux attach -t X`）。
2. 用户显式批准过该命令前缀，且 cwd 与环境摘要一致。
3. 命令来自已信任的 `.cmux/cmux.json` action fingerprint。
...
- `cmux surface resume show --json` 可看到 binding。
...
- 在 `.cmux/cmux.json` 里定义“前端 dev + 浏览器预览 + shell”的三栏布局。
...
| M3-T03 | `BrowserPaneViewModel` | URL、Title、CanGoBack、CanGoForward、Loading | `src/Cmux/ViewModels/BrowserPaneViewModel.cs` |
...
| M3-T06 | CLI browser open | `cmux browser open|open-split|new <url>` | `Cmux.Cli/Program.cs`、`MainViewModel.cs` |
| M3-T07 | cmux.json browser surface | `type:"browser"` parser | `CmuxJsonService.cs` |
...
cmux browser open-split http://localhost:3000 --direction right
cmux browser new http://localhost:5173 --workspace workspace:1
```
...
- `cmux browser open-split http://localhost:3000` 在当前 pane 右侧打开浏览器。
- 浏览器可后退、前进、刷新、打开 DevTools。
- `.cmux/cmux.json` workspace layout 包含 browser surface 可创建成功。
- 关闭重开后恢复浏览器 URL。
...
| 层 | 职责 |
|---|---|
| CLI | `cmux browser snapshot/click/fill/eval/screenshot/...` |
...
从“少量 CLI 命令”升级为稳定 automation surface，让 agent 和脚本能可靠控制 cmux，而不依赖当前焦点窗口。

### M5.2 v2 协议设计

建议使用同一 Named Pipe：`\\.\pipe\cmux`，通过首行 JSON 判断 v2。
...
- `SelectedWorkspace` 不应默认等于用户正在看的窗口；v2 调用应显式指定 target 或由 `CMUX_WORKSPACE_ID` 推断。

### M5.6 验收

- `cmux --json window list` 返回所有窗口。
- `cmux pane write --workspace workspace:2 --pane pane:1 --text ...` 不影响当前用户焦点窗口。
- v1 命令仍可用。
...
让普通 Windows 用户无需手工配置即可把 Claude/Codex/OpenCode 等 agent 接入 cmux 通知与恢复系统，并能通过安装器/自动更新稳定使用。
...
|---|---|---|---|---|
| Claude Code | `where claude` | Claude config JSON / wrapper | `cmux notify` | native session id / wrapper |
| Codex | `where codex` | config / wrapper / PowerShell profile | `cmux notify` | `codex resume` if supported |
| OpenCode | `where opencode` | config | `cmux notify` | native resume if supported |
| Gemini | `where gemini` | wrapper | `cmux notify` | custom |
| Copilot CLI | `where gh` + extension | shell integration | `cmux notify` | custom |
...
3. 标题栏显示“有更新”。
4. 用户点击后下载并提示重启。
5. CLI `cmux update check/install` 可触发更新。

### M6.5 验收

- 新机器安装后 `cmuxw.exe` 可启动，`cmux.exe` 在 PATH 中可用。
- `cmux hooks setup codex` 能检测并写入 hook，失败时打印可操作错误。
- 卸载后 PATH 与 hook 可清理。
- 更新过程不丢 `%LOCALAPPDATA%/cmux` 数据。
...
- 核心能力：terminal/layout/notification/session restore/cmux.json/browser basic/v2 CLI 至少达到 beta 质量。
...
| 场景 | 风险 | 策略 |
|---|---|---|
| 执行 `cmux.json` command | 仓库恶意命令 | 首次执行按 action fingerprint 请求信任 |
...
| `cmux.json` | 不支持字段给 schema warning，不直接崩溃 |
...
| R04 | `cmux.json` 执行恶意命令 | 高 | 中 | 首次运行即执行未知命令 | fingerprint 信任 + confirm |
...
|---|---|
| 首次启动成功率 | > 99% |
| `cmux notify` 到 UI 可见 | < 300ms |
| 10 个 agent session 并行时 UI 无明显卡顿 | 是 |
| 用户从未读通知跳转到目标 pane | 1 次快捷键 |
| `.cmux/cmux.json` 识别并运行项目命令 | 0 配置外步骤 |
...
| Sparkle | Velopack |
| `~/Library/Application Support/cmux` | `%LOCALAPPDATA%/cmux` |
| Unix socket | Named Pipe |
| shell hooks | PowerShell / cmd / agent config / wrapper exe |
| `~/.config/cmux` | `%USERPROFILE%\.config\cmux` |
...
| 6 | `CmuxJsonService` DTO + parser tests | 先做纯 Core，不动 UI |
| 7 | CommandPalette 接入 cmux.json simple command | 打通可见价值 |
...
- 上游官网：<https://cmux.com/zh-CN>
- 自定义命令文档：<https://cmux.com/zh-CN/docs/custom-commands>
- 会话恢复文档：<https://cmux.com/zh-CN/docs/session-restore>
