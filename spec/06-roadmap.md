...
| preview | 每个里程碑结束 | 签名安装器 + zip | 内测用户 |
| stable | `1.0.0` 后按需 | 安装器 + update feed | 普通用户 |
...
- preview/stable：Velopack 安装器。
- 企业分发：MSIX 可选。

### M6.4 更新流程

1. GitHub Release 上传 `RELEASES` feed 与 nupkg / 安装器。
2. App 启动后后台检查版本。
3. 标题栏显示“有更新”。
4. 用户点击后下载并提示重启。
...
- `docs/` 面向中文用户，默认使用**简体中文单语**；不再维护同页中英双语内容。
- CLI 命令、配置字段、协议方法、错误码、文件名和外部产品名保持原文。
- 如后续需要英文站点，应拆为独立 locale / 路由，不在简体中文页面混排英文说明。
...
- docs 以简体中文覆盖安装、配置、CLI、browser API、故障排查。
- 安装器 + 卸载 + 更新流程可用。
...
|---|---|---|---|---|---|
| R01 | ConPTY 在 CI / 旧 Windows 上行为不一致 | 高 | 中 | smoke 测试不稳定 | 限定 Windows 10 1809+；CI 用固定 runner；CI 已加入 `中文 目录/项目/` smoke |
...
| R05 | MSIX 与 ConPTY 不兼容 | 中 | 中 | 安装版无法创建 PTY | 保留 Velopack/Inno 备用路径 |
...
| R09 | Browser automation 被页面 CSP 阻止 | 中 | 中 | ExecuteScriptAsync 失败 | WebView2 DevTools Protocol 备用路径 |
